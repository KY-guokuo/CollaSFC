...
class packet_reconstruction(object):
    ...
    def packet_assembly_3(self, last_node_info_tuple, node_vnf_info_tuple):
        int_index = 2 + self.pkt[INT_HEADER].hop_cnt * 28 - 1
        # self.pkt[UDP].payload)[:int_index + 1]---> int_header and int_data
        # self.pkt[UDP].payload)[int_index + 2:]---> last k8s data
        # vnf_cnt数量增加1
        ...
        pkt2 = layer_assembly(self.pkt, self.src_mac, self.dst_mac, self.header_len_offset) / \
               raw(self.pkt[UDP].payload)[:int_index + 1] / KUBE_HEADER(vnf_cnt=vnf_cnt) / \
               KUBE_DATA(parser_kube_flag=0, node_status=last_node_info_tuple[0],
                         node_cpu_utilization=last_node_info_tuple[1],
                         node_memory_utilization=last_node_info_tuple[2],
                         vnf_status=last_node_info_tuple[3], vnf_cpu_utilization=last_node_info_tuple[4],
                         vnf_memory_utilization=last_node_info_tuple[5]) / \
               KUBE_DATA(parser_kube_flag=0, node_status=node_vnf_info_tuple[0],
                         node_cpu_utilization=node_vnf_info_tuple[1], node_memory_utilization=node_vnf_info_tuple[2],
                         vnf_status=node_vnf_info_tuple[3], vnf_cpu_utilization=node_vnf_info_tuple[4],
                         vnf_memory_utilization=node_vnf_info_tuple[5]) / \
...
# insert the first node information, such as node6. The information of vnf is none.
def insert_k8s_message_1(json_file, k8s_node_name):
    node_status = json_file["node_status"][k8s_node_name]["status"]
    node_cpu_utilization = json_file["node_metrics"][k8s_node_name]["cpu"]
    node_memory_utilization = json_file["node_metrics"][k8s_node_name]["memory"]
    vnf_status = -1
    vnf_cpu_utilization = -1
    vnf_memory_utilization = -1
    return node_status, node_cpu_utilization, node_memory_utilization, vnf_status, \
        vnf_cpu_utilization, vnf_memory_utilization


# we insert the information of node1, node2 or node4, and vnf2 or vnf3.
def insert_k8s_message_2(json_file, k8s_node_name, vnf_name):
    node_status = json_file["node_status"][k8s_node_name]["status"]
    node_cpu_utilization = json_file["node_metrics"][k8s_node_name]["cpu"]
    node_memory_utilization = json_file["node_metrics"][k8s_node_name]["memory"]
    vnf_status = json_file["pod_status"][vnf_name]["status"]
    vnf_cpu_utilization = json_file["pod_metrics"][vnf_name]["cpu"]
    vnf_memory_utilization = json_file["pod_metrics"][vnf_name]["memory"]
    return node_status, node_cpu_utilization, node_memory_utilization, vnf_status, \
        vnf_cpu_utilization, vnf_memory_utilization
        ...
        with open('computing_resource.json', encoding='utf-8') as json_obj:
            ...
            if pkt[IP].tos == 5:
                ...
                if pkt[INT_HEADER].first_vnf_flag == 1:
                    ...
                    node_vnf_info_tuple = insert_k8s_message_2(computing_data, "k8s-node1", vnf_name)
                    ...
                    # 非第一个vnf时，除去kube_header,
                    # k8s-node2 data, vnf3 and k8s-node4, 需增加kube_data的长度为34Bytes--------------------------
                    node_vnf_info_tuple = insert_k8s_message_2(computing_data, "k8s-node2", vnf_name)
                    last_vnf_info_tuple = insert_k8s_message_1(computing_data, "k8s-node4")
                    pc = packet_reconstruction(pkt, src_mac, dst_mac, header_len_offset=34)
                    pc.packet_assembly_3(last_vnf_info_tuple, node_vnf_info_tuple)
            if pkt[IP].tos == 9:
                # 根据INT_HEADER中的first_vnf_flag进行头部堆栈，如果为第一个VNF，则需要增加kube_header和kube_data,
                # kube_data总长度为18Bytes + 17Bytes = 35Bytes.
